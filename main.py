...
from relation import CLIPRelationModuleAgentV2
from distill import CLIPDualMiLoss
import clip

from utils import CompleteLogger, TensorboardWriter
from engine import GeneralMovingAverage, get_dataset, get_text_features, train, evaluate_all, evaluate_all1
...
def main(args: argparse.Namespace):
    ...
    relation_model = CLIPRelationModuleAgentV2().to(device)
    distill_loss = CLIPDualMiLoss().to(device)
    ...
    frozen_model = copy.deepcopy(classifier)  # 深拷贝，独立模型
    for param in frozen_model.parameters():
        param.requires_grad = False  # 只冻结 frozen_model
    clip.model.convert_weights(classifier)
    classifier.eval()  # classifier 用于训练
    
    # obtain text features
    text_features_agent, train_text_features = get_text_features(clip_model, template, train_class_names, device)
    print("Text features obtained for training classes." )
    print(text_features_agent.shape, train_text_features.shape)
    for test_loader in test_loaders:
        _,test_loader["text_features"] = get_text_features(clip_model, template, test_loader["class_names"], device)
    ...
    if args.phase == "train":
        ...
        optimizer = AdamW(
            list(classifier.parameters()) + list(relation_model.parameters()) + list(distill_loss.parameters()),
        ...
        best_val_acc1 = evaluate_all1(classifier, val_loader, train_text_features, test_loaders, args, writer, device)
        ...
        for epoch in range(args.epochs):
            ...
            # train for one epoch
            train(text_features_agent, distill_loss, frozen_model, relation_model, 
                  train_iter, classifier, bma_classifier, train_text_features, 
                  optimizer, lr_scheduler, epoch, args, writer, device)
            # evaluate all
            val_acc1 = evaluate_all1(classifier, val_loader, train_text_features, test_loaders, args, writer, device)
            evaluate_all(frozen_model,classifier, val_loader, train_text_features, test_loaders, args, writer, device)
    ...
    evaluate_all1(classifier, val_loader, train_text_features, test_loaders, args, writer, device)
    evaluate_all(frozen_model,classifier, val_loader, train_text_features, test_loaders, args, writer, device)

    torch.save(bma_classifier.state_dict(), logger.get_checkpoint_path('bma'))
    print("Evaluate BMA model:")
    evaluate_all1(bma_classifier, val_loader, train_text_features, test_loaders, args, writer, device)
    evaluate_all(frozen_model,bma_classifier, val_loader, train_text_features, test_loaders, args, writer, device)
